...
export const JOB_STATUSES = ['感兴趣', '已投递', 'OA / 笔试', 'AI 面试', '一面中', '二面中', '三面中', '终面中', 'Offer', '已结束']
export const APPLIED_STATUSES = ['已投递', 'OA / 笔试', 'AI 面试', '一面中', '二面中', '三面中', '终面中', 'Offer', '已结束']
...
export const JOB_STATUS_TRANSITION_ERROR = '已收到 Offer 或已结束的岗位不能再修改状态；已投递及之后不能改回感兴趣，收到 OA / 笔试、AI 面试或正式面试后不能退回已投递，正式面试轮次不能后退，除终面外不能跨级'
const FINAL_JOB_STATUS_SET = new Set(FINAL_JOB_STATUSES)
const CANNOT_RETURN_TO_APPLIED_STATUSES = new Set(['OA / 笔试', 'AI 面试', ...INTERVIEW_STATUS_ORDER])
...
export function canSelectJobStatus(job, targetStatus) {
  ...
  if (CANNOT_RETURN_TO_APPLIED_STATUSES.has(job?.status) && targetStatus === '已投递') return false
