...
const COLUMNS = [
  ...
  { key: 'AI 面试', color: 'border-t-amber-500/40', headerColor: 'text-amber-400', bgColor: 'bg-amber-500/10' },
...
export default function Board({ jobs: propJobs, isReadOnly = false }) {
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    const savedJob = await updateJob(jobId, patch, job.updatedAt)
    if (!savedJob) return
  ...
  const saveFollowUp = async () => {
    if (isReadOnly) return
    if (!followUpJob) return
    const savedJob = await updateJob(followUpJob.id, { nextAction: followUpText }, followUpJob.updatedAt)
    if (!savedJob) return
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              <button
                ...
                className={`whitespace-nowrap rounded-lg border px-3 py-2 text-xs font-semibold transition-all cursor-pointer ${col.headerColor} ${
                  isActive
                ...
                <div className="flex shrink-0 items-center justify-between px-4 pb-2 pt-3">
                  ...
                  {!isReadOnly && (
                    <button
                      ...
                      className="w-6 h-6 rounded-lg flex items-center justify-center text-offer-muted hover:text-white hover:bg-white/10 transition-all cursor-pointer"
                      title={`新增 ${col.key}`}
                    >
    ...
    >
      ...
      {!isReadOnly && (
        <button
          ...
          className="absolute top-3 right-3 flex h-6 w-6 items-center justify-center rounded-lg text-gray-500 opacity-100 transition-all hover:bg-white/10 hover:text-white dark:text-white/45 md:opacity-0 md:group-hover:opacity-100 cursor-pointer"
        >
...
function MenuItem({ icon, label, onClick, danger }) {
  return (
    <button
      ...
      className={`cursor-pointer w-full flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors ${danger ? 'text-red-500 hover:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/10' : 'text-slate-700 hover:bg-slate-100 dark:text-white/80 dark:hover:bg-white/10'}`}
    >
